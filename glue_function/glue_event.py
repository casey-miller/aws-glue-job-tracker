...
def lambda_handler(event, context):
    ...
    get_glue_job = glue.get_job_run(JobName=event_job_name, RunId=event_job_run_id)
    ...
    sns_flagged = (
        "The Glue job: {} for job run id {} completed successfully but exceeded threshold(s), please refer to below and adjust accordingly.\n\n"
        "Provisioned workers = {}.\n"
        "Worker threshold = {}.\n"
    ...
    sns_alert = (
        "The Glue job: {} for job run id {}: is in the status of {} and run attempt #{}.\n\n"
        "Follow this link to go to the job run - {}".format(
    ...
    def send_sns(message):
        sns.publish(
            TopicArn=SNS_TOPIC, Message=message, Subject="AWS Glue Notification"
        )
    ...
    try:
        if "remediate" in job_tags["Tags"] and job_tags["Tags"]["remediate"] == "false":
            print("ignoring job")
            return {"status": "PASS", "job_name": job_name}
        elif (
            num_workers > int(WORKER_THRESHOLD) or job_timeout > int(DURATION_THRESHOLD)
        ) and job_state == "SUCCEEDED":
            print(
                "Workers and/or job duration is over allowed threshold. The job Succeeded, but notifying about the threshold discrepancy..."
            )
            ...
            print(
                "Glue job {} is in {} state, sending alert...".format(
                    job_name, job_state
                )
            )
            send_sns(sns_alert)
